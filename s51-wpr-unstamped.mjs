  ...
  for (const t of pop) {
    ...
    if (!anyNz) {
      ...
      if (unit) {
        ...
        const changed = old.price !== t.price;
        Object.assign(rec, { disposition: changed ? 'unit-repriced' : 'unit-unchanged', new: t.price, label: floor.singular, unit: unit.unit, rule: unit.rule }); bump(changed ? 'unit-repriced' : 'unit-unchanged'); ladderNote(floor.singular);
      } else {
